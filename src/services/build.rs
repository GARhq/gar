...
pub fn stage_generation(
    ...
    std::fs::write(generation_dir.join(".init_path"), artifact.init_path.display().to_string())?;
    if artifact.kernel_params.exists() {
        std::fs::copy(&artifact.kernel_params, generation_dir.join(".kernel_params"))?;
    }
